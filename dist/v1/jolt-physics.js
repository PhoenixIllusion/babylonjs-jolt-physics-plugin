...
import { CreateJoltConstraint } from '../jolt-constraint';
...
export class JoltJSPlugin {
    ...
    constructor(jolt, _useDeltaForWorldStep = true) {
        ...
        this._raycaster = new RayCastUtility(jolt, {
            world: this.world,
            GetBodyForBodyId: (seqAndNum) => this.GetImpostorForBodyId(seqAndNum).physicsBody,
            GetPhysicsBodyForBodyId: (seqAndNum) => this.GetImpostorForBodyId(seqAndNum)
        });
    ...
    */
    ...
    generateJoint(impostorJoint) {
        ...
        if (nativeParams && nativeParams.constraint) {
            constraint = CreateJoltConstraint(mainBody, connectedBody, nativeParams.constraint);
        }
        else {
